    ...
    <div class="card-body">
      <form action="{{ route('spiritual_backgrounds.update', $member->id) }}" method="POST">
          ...
          <div class="form-group row">
              ...
              <div class="col-md-6">
                <label for="gotram">{{translate('Gotram')}}</label>
                <input type="text" name="gotram" value="{{!empty($member->spiritual_backgrounds->gotram) ? $member->spiritual_backgrounds->gotram : "" }}" class="form-control" placeholder="{{translate('Gotram')}}">
                @error('gotram')
                    <small class="form-text text-danger">{{ $message }}</small>
                @enderror
            </div>
